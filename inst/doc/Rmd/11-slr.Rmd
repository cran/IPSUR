...
of \(x\). This turns out to be a difficult problem[^slr-1], so
instead we first tackle an easier problem: we estimate \(\mathbb{E}Y\). How can we accomplish this? Well, we know that \(Y\) depends
somehow on \(x\), so it stands to reason that
\begin{equation}
\mathbb{E} Y = \mu(x),\ \mbox{a function of }x.
\end{equation}

[^slr-1]: Yogi Berra once said, "It is always difficult to make
...
\@ref(cha-multivariable-distributions) that if \((X,Y) \sim \mathsf{mvnorm}\) then the mean of \(Y|x\) is a linear function of
...
```{r, echo=FALSE,label="philosophy",fig.cap='(ref:cap-philosophy)'}
...
[@datasets]. It has two variables: `speed` and `dist`. We can take
...
```{r,label="carscatter",fig.cap='(ref:cap-carscatter)'}
...
line? We will use maximum likelihood \index{maximum likelihood} to find it. We know:
\begin{equation}
...
the \(Y\) and \(x\) data, respectively. See Exercise
\@ref(xca:show-alternate-slope-formula). Also, notice the similarity
...
`speed`. The `data = cars` argument tells R where to look
...
```{r,label="carline",fig.cap='(ref:cap-carline)'}
plot(dist ~ speed, data = cars)
abline(coef(cars.lm))
```
...
`cars` data (which we saw in Example \@ref(ex:speed-and-stopping)).
...
value we computed by hand in Example \@ref(ex:regline-cars-pe-8mph).
...
[^slr-3]: Be careful not to confuse the mean square error \(S^{2}\)
...
found it to be approximately \(\hat{\mu}(8) \approx\) `r tmpred`. Now, it turns out that there was only one recorded observation
at \(x = 8\), and we have seen this value in the output of
`head(cars)` in Example \@ref(ex:speed-and-stopping); it was 
\(\mathtt{dist} = 16\) ft for a car with \(\mathtt{speed} = 8\) mph. Therefore, the
residual should be \(E = Y - \hat{Y}\) which is \(E \approx 16 -\)
`r tmpred`. Now take a look at the last entry of
...
`summary(cars.lm)` output (see How to do it with R in
Section \@ref(sub-slr-interval-est-params)). We may read it from there
to be \(S\approx\) `r tmps`, or we can access it directly from the
`summary` object.
...
\(b_{1} =\) `r A[2,1]` and \(S_{b_{1}} =\) `r A[2,2]` together with a \(\mathsf{t}_{0.025}(\mathtt{df}=23)\)
...
With 95% confidence, the random interval `r B[2,1]` to
`r B[2,2]` covers the parameter \(\beta_{1}\).
...
value of \(Y\) at \(x_{0}\) -- called a *confidence interval for the mean value of* 
\(Y\) *at* \(x_{0}\) -- than it is to get bounds on a
...
Next we instruct R to calculate the intervals. Confidence
...
1. Report a point estimate of and a 95% confidence interval for the
   ...
   value for \(x = 5\) is `r carsCI[1,1]`, so a point
   estimate would be `r carsCI[1,1]` ft. The 95% CI is
   given by `r carsCI[1,2]` to `r carsCI[1,3]`, so
   with 95% confidence the mean stopping distance lies somewhere
   between `r carsCI[1,2]` ft and `r carsCI[1,3]` ft.
2. Report a point prediction for and a 95% prediction interval for the
   ...
   fitted value for \(x = 21\) is `r carsPI[3,1]`, so a point
   prediction for the stopping distance is `r carsPI[3,1]` ft. The
   95% PI is `r carsPI[3,2]` to `r carsPI[3,3]`, so with
   ...
   `r carsPI[3,2]` ft and `r carsPI[3,3]` ft.
...
```{r,label="carscipi",fig.cap='(ref:cap-carscipi)'}
...
linear regression model is doing, and the *simple coefficient of determination* is designed for that purpose. In what follows, we
...
\end{equation}
We have seen the \(SSE\) before, which stands for the *sum of squared errors* or 
*error sum of squares*. Of course, we would expect the
...
*unexplained variation* (\(SSE\)). In this way we see that the ANOVA equality
...
`summary(cars.lm)` output where it was called `Multiple R-squared`. Listed right beside it is the `Adjusted R-squared` which
...
\@ref(sub-slr-hypoth-test-params) how to test \(H_{0}:\beta_{1} =
...
Here we see that the \(F\) statistic is `r tmpf` with a
...
first examine a histogram in Figure
\@ref(fig:normal-hist-cars), which was produced by the following code. 

```{r,label="normal-hist-cars",fig.cap='(ref:cap-normal-hist-cars)'}
...
the command `qqPlot(cars.lm)` from the `RcmdrMisc` package [@RcmdrMisc], and the results are in Figure
...
```{r,label="normal-q-q-plot-cars",fig.cap='(ref:cap-normal-q-q-plot-cars)',message=FALSE}
library(RcmdrMisc)
qqPlot(cars.lm)
```
...
function from the `stats` package [@stats]. The hypotheses are \[
...
*standardized residuals* \(R_{i}\), \(i=1,2,\ldots,n\), by
\begin{equation} 
...
\@ref(fig:std-resids-fitted-cars). For these data there does appear to
be somewhat of a slight fanning-out of the residuals.

```{r,label="std-resids-fitted-cars", fig.cap='(ref:cap-std-resids-fitted-cars)'}
...
R with the `bptest` function from the `lmtest` package
[@lmtest].
...
examine the residual plot in Figure
\@ref(fig:resids-fitted-cars). There is no obvious cyclical wave
pattern or structure to the residual plot.

```{r, label="resids-fitted-cars",fig.cap='(ref:cap-resids-fitted-cars)'}
...
`dwtest` function from the `lmtest` package [@lmtest]. We will
...
* **Errors are not independent** There is a large class of
     ...
     the latter part of Chapter \@ref(cha-time-series).
...
\(DFBETAS\) with the `dfbetas` function (some output has been
omitted):
...
calculated[^slr-4] by the formula
\begin{equation}
...
[^slr-4]: Cook's distances are actually defined by a different
...
```{r,label="cooks-distance-cars",fig.cap='(ref:cap-cooks-distance-cars)'}
plot(cars.lm, which = 4)
```
...
```{r, label="diagnostic-plots-cars",fig.cap='(ref:cap-diagnostic-plots-cars)'}
par(mfrow = c(2,2))
plot(cars.lm)
par(mfrow = c(1,1))
```
...
```{block, type="xca", label="xca-show-alternate-slope-formula"}
