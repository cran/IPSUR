# Discrete Distributions {#cha-discrete-distributions}
...
\@ref(sub-kolmogorov-smirnov-goodness-of-fit-test) where we discuss
...
Discrete Distributions* by Johnson *et al*[@Johnson1993].
...
## Discrete Random Variables {#sec-discrete-random-variables}


### Probability Mass Functions {#sub-probability-mass-functions}
...
\@ref(cha-probability) that PMFs enjoy certain properties. In particular, all
PMFs satisfy
...
Table: Flipping a coin three times: the PMF.

### Mean, Variance, and Standard Deviation {#sub-mean-variance-sd}
...
```{example, label="disc-pmf-mean"}
We will calculate the mean of \(X\) in Example
\@ref(ex:toss-a-coin).  \[ \mu = \sum_{x = 0}^{3}xf_{X}(x) = 0 \cdot
...
\@ref(sec-interpreting-probabilities)). Another way it is commonly stated is
...
#### How to do it with R {#sub-disc-rv-how-r}


The mean and variance of a discrete random variable is easy to compute
at the console. Let's return to Example \@ref(ex:disc-pmf-mean). We will start
...
R since operations on vectors are performed
*element-wise* (see Section \@ref(sub-functions-and-expressions)):
...
[@distrEx]. We define a random variable `X` as an object, then
...
## The Discrete Uniform Distribution {#sec-disc-uniform-dist}
...
**Examples.** 
To roll a fair die 3000 times, do `sample(6, size = 3000, replace = TRUE)`. To choose 27 random numbers from 30 to 70, do `sample(30:70, size = 27, replace = TRUE)`. To flip a fair coin 1000 times, do `sample(c("H","T"), size = 1000, replace = TRUE)`.
...
### The Binomial Model {#sub-the-binomial-model}
...
The corresponding R function for the PMF and CDF are
...
probabilities...` menu in the R Commander or the
...
```{example, label="toss-coin-3-withr"}
...
```{r, echo=FALSE,label="binom-cdf-base", fig.cap='(ref:cap-binom-cdf-base)'}
...
(ref:cap-binom-cdf-base) \small A graph of the \(\mathsf{binom}(\mathtt{size}=3,\,\mathtt{prob}=1/2)\) CDF. 
...
from the `base` package are specified by capitalizing the
...
Random variables defined via the `distr` package [@distr] may be
...
```{r, echo=FALSE,label="binom-plot-distr",fig.cap='(ref:cap-binom-plot-distr)'}
X <- Binom(size = 3, prob = 1/2)
plot(X, cex = 0.2)
```

(ref:cap-binom-plot-distr) \small The \textsf{binom}(`size` = 3, `prob` = 0.5) distribution from the `distr` package.
...
Table: Correspondence between `stats` and `distr`. We are given \(X\sim\mathsf{dbinom}(\mathtt{size}=n,\,\mathtt{prob}=p)\).  For the `distr` package we must first set \(\mathtt{X\ <-\ Binom(size=}n\mathtt{,\ prob=}p\mathtt{)}\).

## Expectation and Moment Generating Functions {#sec-expectation-and-mgfs}


### The Expectation Operator {#sub-expectation-operator}

We next generalize some of the concepts from Section
\@ref(sub-mean-variance-sd). There we saw that every[^discdist-1] PMF has
...
[^discdist-1]: Not every, only those PMFs for which the (potentially infinite) series converges.
...
\end{equation}
in Exercise \@ref(xca-variance-shortcut). Intuitively, for repeated observations
...
```{proposition, label="expectation-properties"}
...
### Moment Generating Functions {#sub-mgfs}
...
[^discdist-2]: We are glossing over some significant mathematical details in our derivation. Suffice it to say that when the MGF exists in a neighborhood of \(t=0\), the exchange of differentiation and summation is valid in that neighborhood, and our remarks hold true.
...
The `distrEx` package [@distrEx] provides an expectation operator
...
## The Empirical Distribution {#sec-empirical-distribution}
...
Chapter \@ref(cha-hypothesis-testing)) or resampling (see Chapter
...
```{r, label="empirical-cdf",fig.cap='(ref:cap-empirical-cdf)'}
plot(ecdf(x))
```

(ref:cap-empirical-cdf) \small The empirical CDF.


See Figure \@ref(fig:empirical-cdf). The graph is of a
right-continuous function with jumps exactly at the locations stored
in `x`. There are no repeated values in `x` so all of the jumps are
equal to \(1/5=0.2\).
...
`quantile(x, probs = p, type = 1)`; see Section \@ref(sub-normal-quantiles-qf).
...
## Other Discrete Distributions {#sec-other-discrete-distributions}
...
### Dependent Bernoulli Trials {#sec-non-bernoulli-trials}

#### The Hypergeometric Distribution {#sub-hypergeometric-dist}
...
\@ref(sub-normal-quantiles-qf), and `rhyper`, discussed below.
...
\choose 2}}{{250 \choose 5}}.  \] To calculate it in R we
just type
...
be to add the 0, 1, and 2 values returned from `dhyper`. 

```{r, echo = TRUE}
sum(dhyper(0:2, m = 17, n = 233, k = 5))
```

But another way would be just to use `phyper` and be done with it.
...
**Find \(\mathbb{P}(X>1)\).**  Since
\(\mathbb{P}(X>1)=1-\mathbb{P}(X\leq1)=1-F_{X}(1)\), we can find the
probability with the `lower.tail` argument to `phyper`.

```{r, echo=TRUE}  
phyper(1, m = 17, n = 233, k = 5, lower.tail = FALSE)
```

In general, the `phyper(x, ..., lower.tail = FALSE)` command  
computes \(\mathbb{P}(X > x)\) for given \(x\).  

**Generate \(100,000\) observations of the random variable \(X\).**  Here `rhyper` comes to he rescue.

```{r, echo=TRUE}  
x <- rhyper(100000, m = 17, n = 233, k = 5)
```

Our simulations are stored in the vector `x`.  We could take a look at a histogram of `x` if we like (omitted). We know from our formulas that \(\mu=K\cdot
...
From the output above our estimate is `r sd(x)`, and from our formulas we get
\[
...
#### Sampling With and Without Replacement {#sub-sampling-with-and}
...
### Waiting Time Distributions {#sec-waiting-time-distributions}
...
#### The Geometric Distribution {#sub-the-geometric-distribution}
...
\(X\sim\mathsf{geom}(\mathtt{prob}=p)\). The associated R
...
#### The Negative Binomial Distribution {#sub-the-negative-binomial}
...
associated R functions are `dnbinom(x, size, prob)`,
...
### Arrival Processes {#sec-arrival-processes}

#### The Poisson Distribution {#sub-the-poisson-distribution}
...
R functions are `dpois(x, lambda)`, `ppois`, `qpois`, and
...
```{block, type="remark", label="rem-poisson-process"}
...
## Functions of Discrete Random Variables {#sec-functions-discrete-rvs}
...
\@ref(sec-differential-and-integral)), the solution can be found by simple
substitution.
...
saw in \@ref(sec-other-discrete-distributions) that \(X\) represents the number
...
Table: Transforming a discrete random variable.
...
Table: Transforming a discrete random variable, its PMF.
...
```{block, type="xca", label="xca-variance-shortcut"}
...
```{block, type="xca", label="xca-binom-factorial-expectation"}
If
...
```{block, type="xca", label="xca-hyper-mean-variance"}
