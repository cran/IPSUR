...
\@ref(cha-sampling-distributions) about sampling
...
```{example, label="how-many-fish", name="Fishing, part one"}
...
```{r, echo=FALSE, label="capture-recapture",fig.cap='(ref:cap-capture-recapture)'}
heights = rep(0, 16)
...
```{example, label="bass-bluegill", name="Fishing, part two"}
...
```{r, echo=FALSE, label="fishing-part-two",fig.cap='(ref:cap-fishing-part-two)'}
...
value \(\hat{p}=\overline{x}\) is located at a minimum[^est-2]
...
```{r, echo=FALSE, label="species-mle",fig.cap='(ref:cap-species-mle)'}
...
```{example, label="normal-mle-both"}
Let \(X_{1}\), \(X_{2}\), ... , \(X_{n}\) be
an \(SRS(n)\) from a
\(\mathsf{norm}(\mathtt{mean}=\mu,\,\mathtt{sd}=\sigma)\)
distribution. It can be shown (in Exercise \@ref(xca-norm-mu-sig-mle)) that
...
`r round(amax, 3)` and `objective` gives `L` evaluated at the
...
R. We just need to calculate the log-likelihood
...
the `mle` function in the `stats4` package [@stats4].
...
There is functionality in the `distrTEst` package to
...
```{r, label="ci-examp",fig.cap='(ref:cap-ci-examp)'}
ci.examp()
```
...
weights[^estimate-03]. We know the population standard deviation
\(\sigma=0.70\) from prior research. We are going to use the
one-sample \(z\)-interval.

[^estimate-03]: Actually we will see later that there is reason to believe
...
now -- we will use it in Chapter \@ref(cha-hypothesis-testing)). We can make the
plot for Figure \@ref(fig:plant-z-int-plot) with
...
* If one or more of the sample sizes is small then we are in trouble,
  ...
  is called the "pooled" estimator of \(\sigma\).
    * If one of the samples is small, and both underlying populations
      ...
      freedom. See Welch [@Welch1947], Satterthwaite
      [@Satterthwaite1946], or Neter *et al* [@Neter1996]. The
...
2. Go back to first principles. Note that \[
   ...
   \@ref(cha-categorical-data-analysis). It is also known as the *Wilson
   interval*; see Agresti [@Agresti2002].
...
[@TeachingDemos] and `var.test` in base R [@base]
here.
...
Sections \@ref(sec-confidence-intervals-for-means) through \@ref(sec-confidence-intervals-for-variances) all began the same way: we were given the sample size
...
```{block, type="xca", label="xca-norm-mu-sig-MLE"}
