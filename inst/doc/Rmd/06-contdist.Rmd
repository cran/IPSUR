...
function* (PDF) denoted \(f_{X}\) associated with it[^contdist-1] that
satisfies three basic properties:
1. \(f_{X}(x)>0\) for \(x\in S_{X}\),
2. \(\int_{x\in S_{X}}f_{X}(x)\,\mathrm{d} x=1\), and
3. \label{enu-contrvcond3} \(\mathbb{P}(X\in A)=\int_{x\in
...
\((F_{X})'=f_{X}\) in the continuous case[^contdist-2].

[^contdist-2]: In the discrete case, \(f_{X}(x)=F_{X}(x)-\lim_{t\to
x^{-}}F_{X}(t)\).
...
packages!distr@\texttt{distr}} package [@distr] way.
...
Compare this to the answer we found in Example \@ref(ex:cont-pdf3x2). We could
...
Compare this to the answer we got in Example \@ref(ex:cont-pdf-3x4). Use `-Inf`
for \(-\infty\).
...
\@ref(sub-disc-rv-how-r) in Chapter
\@ref(cha-discrete-distributions). We define an absolutely continuous
random variable:
...
expectation with the `distrEx` package [@distrEx]:
```{r, echo=TRUE} 
E(X); var(X); 3/80
```

Compare these answers to the ones we found in Example
\@ref(ex:cont-pdf3x2). Why are they different? Because the `distrEx`
...
the associated R function is `dnorm(x, mean = 0, sd = 1)`.
...
closed room, to the journey of a bottle floating in the ocean, to the white
...
The answer is shown in `$root` which is approximately `r temp`, that
...
[^contdist-3]: The precise definition of the quantile function is
...
2. \(Q_{X}\) is left-continuous (see Appendix
   \@ref(sec-differential-and-integral)). For discrete random variables
...
```{proposition, label="func-cont-rvs-pdf-formula"}
...
```{example, label="lnorm-transformation"}
Let
\(X\sim\mathsf{norm}(\mathtt{mean}=\mu,\,\mathtt{sd}=\sigma)\), and
...
```{block, type="fact", label="fac-lin-trans-norm-is-norm"}
If
...
```{proposition, name="The Probability Integral Transform"}
...
The `distr` package [@distr] has functionality to investigate
...
`distr` package can handle the transformation in Example
...
distribution; see Section \@ref(sec-other-continuous-distributions)). A
...
The warning (not shown here) confirms that the `d-p-q` functions are not calculated
...
\end{equation}
The associated R functions are `dexp(x, rate = 1)`,
...
The associated R functions are `dgamma(x, shape, rate = 1)`, `pgamma`, `qgamma`, and `rgamma`, which give the PDF, CDF,
...
associated R functions are `dchisq(x, df)`, `pchisq`,
...
```{r, label="chisq-dist-vary-df",fig.cap='(ref:cap-chisq-dist-vary-df)'}
...
m,\,\mathtt{scale} = \beta)\). The associated R function
...
associated R function is `dbeta(x, shape1, shape2)`. The
mean and variance are
\begin{equation} 
\mu=\frac{\alpha}{\alpha+\beta}\mbox{ and }\sigma^{2}=\frac{\alpha\beta}{\left(\alpha+\beta\right)^{2}\left(\alpha+\beta+1\right)}.
\end{equation}
See Example \@ref(ex:cont-pdf3x2). This distribution comes up a lot in Bayesian
...
associated R function is `dlogis(x, location = 0, scale = 1)`. The logistic distribution comes up in differential equations as a
...
associated R function is `dlnorm(x, meanlog = 0, sdlog = 1)`. Notice that the support is concentrated on the positive \(x\)
...
associated R function is `dweibull(x, shape, scale = 1)`.
...
package [@actuar]. The convention is `m` in front of the
...
```{r, label="gamma-mgf", fig.cap='(ref:cap-gamma-mgf)'}
...
```{block, type="xca", label="xca-prove-the-memoryless"}
