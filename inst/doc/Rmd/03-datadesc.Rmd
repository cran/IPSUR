...
package [@datasets] (which ships with base R). See
...
#### Strip charts also known as Dot plots {#par-strip-charts}
...
```{r, label="stripcharts",echo=FALSE, fig.cap='(ref:cap-stripcharts)'} 
...
(ref:cap-stripcharts) \small Three stripcharts of three data sets.  The first graph uses the `overplot` method, the second the `jitter` method, and the third the `stack` method.
...
```{r, label="histograms", echo=FALSE, fig.cap='(ref:cap-histograms)'}
par(mfrow = c(1,2))
...
```{r, label="histograms-bins", echo=FALSE, fig.cap='(ref:cap-histograms-bins)'}
par(mfrow = c(1,3))
...
(ref:cap-histograms-bins) \small More histograms of the `precip` data.
...
stored in the time series `LakeHuron`. \index{Data
...
```{r, echo=FALSE, fig.cap='(ref:cap-lakehuron)',fig.height=8}
par(mfrow = c(3,1))
...
(ref:cap-lakehuron) \small Index plots of the `LakeHuron` data.
...
```{r,label="bar-gr-stateregion",echo=FALSE, fig.cap='(ref:cap-stateregion)'}
...
(ref:cap-stateregion) \small The top graph is a frequency barplot made with `table` and the bottom is a relative frequency barplot made with `prop.table`.
...
[@RcmdrPlugin.IPSUR] or with the `pareto.chart`
...
```{r, echo=FALSE, label="pareto-chart", fig.cap='(ref:cap-pareto)'}
pareto.chart(table(state.division), ylab="Frequency", cex.lab = cexlab)
```

(ref:cap-pareto) \small Pareto chart of the `state.division` data.
...
```{r, label="dotchart", fig.cap='(ref:cap-dotchart)'}
...
sign of \(g_{2}\). See Tabachnick & Fidell [@Tabachnick2006] for
details.
...
The `e1071` package [@e1071] has the `skewness` function for the
...
## Multivariate Data and Data Frames {#sec-multivariate-data}
...
\@ref(sub-vectors) how to form vectors with the `c` function or the
`scan` function.
...
### Bivariate Data {#sub-bivariate-data}
...
## Comparing Populations {#sec-comparing-data-sets}
...
```{r, echo=FALSE, fig.cap='(ref:cap-xyplot)'}
print(xyplot(Petal.Width ~ Petal.Length, data = iris, group = Species))
```

(ref:cap-xyplot) \small Scatterplot of Petal width versus length in the `iris` data.
...
* Scatterplot matrices
  ...
  + splom( ~ cbind(Fertility,Agriculture,Examination,Education,
  Catholic,Infant.Mortality), data = swiss)
  + splom(~ cbind(Fertility,Agriculture,Examination), 
...
```{r, echo=FALSE, fig.cap='(ref:cap-bwplot)'}
print(bwplot(~weight | feed, data = chickwts))
```

(ref:cap-bwplot) \small Boxplots of `weight` by `feed` type in the `chickwts` data.
...
```{r, echo=FALSE,fig.cap='(ref:cap-histg)'}
print(histogram(~age | education, data = infert))
```

(ref:cap-histg) \small Histograms of `age` by `education` level from the `infert` data.
...
```{r, echo=FALSE,fig.cap='(ref:cap-xyplot-by)'}
print(xyplot(Petal.Length ~ Petal.Width | Species, data = iris))
```

(ref:cap-xyplot-by) \small An `xyplot` of `Petal.Length` versus `Petal.Width` by `Species` in the `iris` data.
...
```{r, echo=FALSE,fig.cap='(ref:cap-coplot)'}
print(coplot(conc ~ uptake | Type * Treatment, data = CO2))
```

(ref:cap-coplot) \small A `coplot` of `conc` versus `uptake` by `Type` and `Treatment`.

## Exercises

Open R and issue the following commands at the command
line to get started. Note that you need to have the
`RcmdrPlugin.IPSUR` package [@RcmdrPlugin.IPSUR] installed, and
for some exercises you need the `e1071` package [@e1071].
...
1. Make boxplots of `salary` by `gender` with the following method:
       ...
       i) On the `Rcmdr`, click `Graphs` \(\triangleright\) `IPSUR - Boxplot...`
       i) In the `Variable` box, select `salary`.
       i) Click the `Plot by groups...` box and select `gender`. Click `OK`.
       i) Click `OK` to graph the boxplot.
