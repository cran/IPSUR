...
```{r, echo=FALSE, label="diagram", fig.cap='(ref:cap-diagram)'}
require(diagram)
par(mex = 0.2, cex = 0.5)
...
(ref:cap-diagram) \small Two types of experiments.


## Sample Spaces {#sec-sample-spaces}
...
package [@prob]. A sample space is (usually) represented by a
*data frame*, that is, a rectangular collection of variables (see
Section \@ref(sub-multivariate-data)). Each row of the data frame corresponds to
an outcome of the experiment. The data frame choice is convenient both
for its simplicity and its compatibility with the R
...
data structure. See Section \@ref(sub-howto-ps-objects) for details.
...
package [@base], `combn` in the `combinat` package
[@combinat], and `permsn` in the `prob` package
[@prob][^prob-1].

[^prob-1]: The seasoned R user can get the job done
...
others with the `nsides` argument. The command `rolldie(3, nsides = 4)` would be used to roll a 4-sided die three times.
...
### Sampling from Urns {#sub-sampling-from-urns}
...
(second) case we say that sampling is done *with (without) replacement*.
...
about the order in which they were selected. We call this *unordered sampling* (in contrast to *ordered*) because the order of the
...
The `prob` package [@prob] accomplishes sampling from urns with
the `urnsamples` \index{urnsamples@\texttt{urnsamples}}
...
```{example, label="sample-urn-two-from-three"}
...
even when, in fact, the call to the function was `urnsamples(..., ordered = FALSE)`. Similar remarks apply for the `replace` argument.

## Events {#sec-events}

An *event* \index{event} \(A\) is merely a collection of outcomes, or
in other words, a subset of the sample space[^prob-2]. After the
...
bunch of events \(A_{1}\), \(A_{2}\), \(A_{3}\), ... are *mutually exclusive* \index{mutually exclusive} or *disjoint* if \(A_{i}\cap
...
[^prob-2]: This na\"{\i}ve definition works for finite or countably
...
Of course, one may ask why we did not try something like `all(y %in% x)`, which would give a single result, `TRUE`. The reason is that the
...
Table: Basic set operations.  The first column lists the name, the second shows the typical notation, the third describes set membership, and the fourth shows how to accomplish it with R.
...
are of class `ps`. See Section \@ref(sub-howto-ps-objects).

\bigskip

```{block, type="note"}
When the `prob` package [@prob] loads you will notice a message:
`The following object(s) are masked from package:base: intersect, setdiff`. The reason for this message is that there already exist
...
is loaded, R recognizes that there are multiple versions
...
## Model Assignment {#sec-interpreting-probabilities}
...
<http://www-history.mcs.st-andrews.ac.uk/Mathematicians/Kolmogorov.html> for more information.
...
*Law of Large Numbers* which may be loosely described as follows. Let \(E\)
...
theory approach. See <http://www-history.mcs.st-andrews.ac.uk/Biographies/Mises.html> for more.
...
[@Mezrich2003] that MIT students were accomplished enough with
...
The roots of subjective probability reach back a long time. See <http://en.wikipedia.org/wiki/Subjective_probability>
...
In the `prob` package [@prob], a probability space is an object of
...
To accomplish this in R, we may use the `probspace`
...
```{example, label="unbalanced-coin",name="An unbalanced coin"} 
While the `makespace`
...
It should be mentioned that while the splendour of R is
...
## Properties of Probability {#sec-properties-of-probability}

### Probability Functions {#sub-probability-functions}
...
```{block, type="axiom", label="ax-prob-nonnegative"}
...
```{block, type="axiom", label="ax-total-mass-one"}
\(\mathbb{P}(S)=1\).
...
```{block, type="axiom", label="ax-countable-additivity"}
...
1. \label{enu-prop-prob-complement} \(\mathbb{P}(A^{c})=1-\mathbb{P}(A)\).
   **Proof:**
...
```{example, label="three-child-family"}
...
Probabilities are calculated in the `prob` package [@prob] with
the `Prob` function.
...
and `given`, which is described in Section \@ref(sec-conditional-probability).
...
other words, `Prob(x, event)` calculates `Prob(intersect(x, subset(x, event)))`.

Consequently, `x` should be the entire probability space in the case
that `event` is non-null.

## Counting Methods {#sec-methods-of-counting}
...
```{proposition, name="The Multiplication Principle"}
...
large. And while R is wonderful and powerful and does
almost everything except wash windows, even R has limits
...
Table: Sampling \(k\) from \(n\) objects with `urnsamples`.
...
\@ref(ex:sample-urn-two-from-three). Recall that we took a sample of size two from an
...
(with `rep = FALSE`, `ord = TRUE`). Each artist will select 4 of his/her
paintings from 7 for display in a row (`rep = FALSE`, `ord = TRUE`),
and lastly, each of the 3 walls has 31 lighting possibilities 
(`rep = TRUE`, `ord = TRUE`). These three numbers can be calculated quickly
with
...
```{example, name="The Birthday Problem"}
...
sorts of complications such as hospital delivery schedules). See <http://en.wikipedia.org/wiki/Birthday_problem>
for more.
...
Now we will use the complementation trick we saw in Example \@ref(ex:three-child-family). We realize that the only situation in which \(A\) does
...
```{r, echo=FALSE, label="birthday",fig.cap='(ref:cap-birthday)'}
g <- Vectorize(pbirthday.ipsur)
plot(1:50, g(1:50), xlab = "Number of people in room", ylab = "Prob(at least one match)")
remove(g)
```

(ref:cap-birthday) \small The birthday problem. The horizontal line is at \(p=0.50\) and the vertical line is at \(n=23\).
...
There is a `Birthday problem` item in the `Probability` menu \newline of
`RcmdrPlugin.IPSUR`. In the base R version, one can
...
then the probability that the second also is an Ace should be \(3/51\), but if the first card is not an Ace, then the probability that the
...
[^prob-3]: Conditional probability in this case is defined by
...
```{example, label="toss-a-six-sided-die-twice"}
...
```{r, echo=FALSE,label="twodiceAB",fig.cap='(ref:cap-twodiceAB)'}
A <- rolldie(2)
B <- subset(A, X1==X2)
C <- subset(A, X1+X2 > 7)
B$lab <- rep("X", dim(B)[1])
...
(ref:cap-twodiceAB) \small Rolling two dice. The outcomes in A are marked with X, the outcomes in B are marked with O.
...
Continuing with Example \@ref(ex:toss-a-six-sided-die-twice), the first thing to do is set
...
```{example, label="two-cards-both-aces"}
...
#### How to do it with R {#sub-howto-ps-objects}

Continuing Example \@ref(ex:two-cards-both-aces), we set up the probability
...
```{example, label="urn-7-red-3-green"}
...
`X1=="red" & X2=="red" & X3=="red"`, but there must
...
## Independent Events {#sec-independent-events}
...
```{example, name="An unbalanced coin"}
...
\@ref(sec-interpreting-probabilities). In this section we introduce a rule that
...
What does it mean? Usually in applications we are given (or know) *a priori* probabilities \(\mathbb{P}(B_{k})\). We go out and collect
...
```{example, label="misfiling-assistants",name="Misfiling Assistants"}
In this problem,
...
Table: Misfiling assistants: workload.
...
Table: Misfiling assistants: prior.
...
Table: Misfiling assistants: misfile rate.
...
Table: Misfiling assistants: posterior.
...
```{example, label="misfiling-assistants-multiple"}
...
package [@prob], but problems like the ones above are easy enough
to do by hand.

\bigskip

```{example, name="Misfiling Assistants"}
...
\@ref(ex:misfiling-assistants). We would replace `prior` with `post`
in a future calculation. We could raise `like` to a power to see how
the posterior is affected by future document mistakes. (Do you see
why? Think back to Section \@ref(sec-independent-events).)
...
Compare this to what we got in Example \@ref(ex:misfiling-assistants-multiple).
...
Table:  Flipping a coin twice.

Taking a look at the second row of the table, we see that the support
of \(X\) -- the set of all numbers that \(X\) assumes -- would be \(S_{X}= \{ 0,1,2 \}\).
...
similar to those of \(X\) and \(Y\) are called *discrete random variables*. 
...
reason[^prob-4], random variables with supports like \(Z\) are
called *continuous random variables*, to be studied in Chapter
\@ref(cha-continuous-distributions).

[^prob-4]: This isn't really the reason, but it serves as an
effective litmus test at the introductory level. See Billingsley [@Billingsley1995] or
Resnick [@Resnick1999].
...
value should be a legitimate function from R, such as
...
```{block, type="xca", label="xca-numb-cond-indep"}
